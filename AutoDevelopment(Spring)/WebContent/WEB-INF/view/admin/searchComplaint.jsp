...
<body class="datatable_page">
	...
	<div id="toast-container" class="toast-top-right">
		<div class="toast toast-success" aria-live="polite"
			...
			<div class="toast-message">Complaint Replyed!</div>
		</div>
	</div>
									...
									<div class="card-block p-t-25">
										...
										<f:form modelAttribute="ls">
											<table class="table table-striped table-bordered table-hover"
												...
												<tbody>
													<c:forEach items="${complaintList}" var="r">



														<tr>
															<td>${r.subject}</td>
															<td>${r.complaintDescription}</td>
															<td>${r.presentdate}</td>
															<td>${r.presenttime}</td>
															<td>${r.complaintStatus}</td>
															<td>${r.complaintReplay}</td>
															<td>${r.complaintTo.loginId }</td>
															<td>${r.complaintFrom.loginId }</td>
															<c:if test="${r.complaintStatus eq  'Pending'}">
																<td><a
																	href="replayComplaint.html?complaintId=${r.complaintId}">Replay</a></td>
															</c:if>
															<c:if test="${r.complaintStatus eq  'Replay'}">
																<td> Replyed</td>
															</c:if>
														</tr>
													</c:forEach>
