...
$(document)
		.ready(
				function() {
					...
					var shortCutFunction = "Helo";
					var msg = "Helo";
					var title = "<span>Helo</span> <h5 class='text-white'>Helo</h5>";
					...
					$("#toastrOptions").text(
							"Command: toastr[" + shortCutFunction + "](\""
									+ msg + (title ? "\", \"" + title : '')
									+ "\")\n\ntoastr.options = "
									+ JSON.stringify(toastr.options, null, 2));
					var $toast = toastr[shortCutFunction](msg, title); // Wire
					// up an
					// event
					// handler
					// to a
					...
					// tabs 1
					// start area chart
					var da1 = [ [ "Jan", 80 ], [ "Feb", 100 ], [ "Mar", 95 ],
							[ "Apr", 105 ], [ "May", 80 ], [ "Jun", 95 ],
							[ "Jul", 80 ], [ "Aug", 90 ], [ "Sep", 115 ],
							[ "Oct", 90 ], [ "Nov", 85 ], [ "Dec", 90 ] ];
					var da2 = [ [ "Jan", 40 ], [ "Feb", 60 ], [ "Mar", 50 ],
							[ "Apr", 60 ], [ "May", 35 ], [ "Jun", 45 ],
							[ "Jul", 30 ], [ "Aug", 40 ], [ "Sep", 60 ],
							[ "Oct", 40 ], [ "Nov", 60 ], [ "Dec", 35 ] ];
					$.plot("#area-chart", [ {
						data : da1,
						label : "Views",
						color : "#69d3be"
					...
					// tab-2
					var realtoggle = new Switchery(document
							.querySelector('#toggle_real'), {
						size : 'small',
						color : '#00c0ef',
						jackColor : '#fff'
					});
					var data_1 = [], totalPoints_1 = 70;
					...
					function getRandomData_1() {
						...
						while (data_1.length < totalPoints_1) {
							var prev_1 = data_1.length > 0 ? data_1[data_1.length - 1]
									: 50;
					...
					var plot_1 = $.plot($("#realtime"), [ getRandomData_1() ],
							options_1);
					...
					$('body')
							.on(
									...
									function() {
										$('#btn_color')
												.removeClass(
														'btn-secondary btn-danger btn-primary btn-info btn-mint')
												.addClass($(this).data('color'));
										$('#btn_color').data('badge',
												$(this).data('badge'));
					...
					$("form#main_input_box")
							.submit(
									function(event) {
										...
										var twoButtons = "<div class='col-lg-3 col-sm-4 col-xs-3 showbtns todoitembtns'>"
												+ editButton
												+ striks
												+ deleteButton + "</div>";
										var badgeClass = $('#btn_color').data(
												'badge');
										$("#custom_textbox").val('');
										$('#btn_color').css("color", "#fff");
										return false;
					...
					$(".todo_section").on(
							...
							'.striked',
							function(e) {
								$(this).closest('.todolist_list').find(
							...
							.on(
									...
									function(e) {
										...
										$(this).closest('.todolist_list').find(
												'.striked').toggle();
										if ($(this).text() == " ") {
											$(this).closest('.todolist_list')
													.find(".showbtns")
													.toggleClass("opacityfull");
											var text1 = $(this)
					...
					new CountUp("visitors_count", 0, 1700, 0, 2.5, options)
							.start();
					new CountUp("revenue_count", 0, 1200, 0, 2.5, options)
							.start();
					new CountUp("subscribers_count", 0, 1020, 0, 2.5, options)
							.start();
					// =========================News
					// ticker===========================================
					...
					var progressBar = {
						...
						progressBar : progressBarContainer
								.find('.progress-bar'),
						progressInfo : progressBarContainer
					...
					var progressBar = {
						...
						progressBar : progressBarContainer12
								.find('.progress-bar'),
						progressInfo : progressBarContainer12
								.children('.progress-primary'),
						set : function(value, info, noPush) {
							if (!noPush) {
								this.chain.push(value);
							}
					...
					var progressBar = {
						...
						progressBar : progressBarContainer2
								.find('.progress-bar'),
						progressInfo : progressBarContainer2
								.children('.progress-primary'),
						set : function(value, info, noPush) {
							if (!noPush) {
								this.chain.push(value);
					...
					var progressBar = {
						...
						progressBar : progressBarContainer1
								.find('.progress-bar'),
						progressInfo : progressBarContainer1
								.children('.progress-info'),
						set : function(value, info, noPush) {
							if (!noPush) {
								this.chain.push(value);
